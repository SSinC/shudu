        ...
        [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
            ...
            if (data && statusCode == 200){
                ...
                if ([NSJSONSerialization class]){
                    json = [[NSJSONSerialization JSONObjectWithData:data options:(NSJSONReadingOptions)0 error:&connectionError][@"results"] lastObject];
                }
                else{
                    //convert to string
                ...
                if (!connectionError){
                    ...
                    if (bundleID){
                        ...
                        if ([bundleID isEqualToString:strongSelf.applicationBundleID]){
                            ...
                            //check version
                            if (strongSelf.onlyPromptIfLatestVersion && !self.previewMode)
                            {
                                ...
                                if ([latestVersion compare:strongSelf.applicationVersion options:NSNumericSearch] == NSOrderedDescending)
                                {
                                    if (strongSelf.verboseLogging)
                                    {
                                        NSLog(@"iRate found that the installed application version (%@) is not the latest version on the App Store, which is %@", strongSelf.applicationVersion, latestVersion);
                                    }
                                    
                                    connectionError = [NSError errorWithDomain:iRateErrorDomain code:iRateErrorApplicationIsNotLatestVersion userInfo:@{NSLocalizedDescriptionKey: @"Installed app is not the latest version available"}];
                                }
                            }
                        }
                        else
                        {
                            if (strongSelf.verboseLogging)
                            {
                                NSLog(@"iRate found that the application bundle ID (%@) does not match the bundle ID of the app found on iTunes (%@) with the specified App Store ID (%@)", self.applicationBundleID, bundleID, @(strongSelf.appStoreID));
                            }
                            
                            connectionError = [NSError errorWithDomain:iRateErrorDomain code:iRateErrorBundleIdDoesNotMatchAppStore userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Application bundle ID does not match expected value of %@", bundleID]}];
                        }
                    }
                    else if (strongSelf.appStoreID || !strongSelf.ratingsURL)
                    {
                        if (strongSelf.verboseLogging)
                        {
                        ...
                        {
                            connectionError = [NSError errorWithDomain:iRateErrorDomain
            ...
            else if (statusCode >= 400)
            {
                ...
                connectionError = [NSError errorWithDomain:@"HTTPResponseErrorDomain" code:statusCode userInfo:@{NSLocalizedDescriptionKey: message}];
            }
            
            //handle errors (ignoring sandbox issues)
            if (connectionError && !(connectionError.code == EPERM && [connectionError.domain isEqualToString:NSPOSIXErrorDomain] &&strongSelf.appStoreID))
            {
//                [strongSelf performSelectorOnMainThread:@selector(connectionError:) withObject:connectionError waitUntilDone:YES];
                [strongSelf connectionError:connectionError];
            }
...
//                [strongSelf connectionSucceeded];
            }
...
{
    NSLog(@"[UIApplication sharedApplication].applicationState:%li",[UIApplication sharedApplication].applicationState);
